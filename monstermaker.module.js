function bootMonsterMaker(){
    const makerPanel = document.getElementById('panel-maker');
    if (!makerPanel) return;
    ...
    */
    ...
    const dataFiles = [
      ...
      const inImageName   = document.getElementById('mm_image_name'); 
      const imgPreviewEl  = document.getElementById('mm_image_preview');
      ...
      // ---------- image ----------
        function normalizeImageBase(raw){
          let s = (raw ?? '').toString().trim();
          if (!s) return '';
  
          // If they paste a full path, keep only the last segment
          s = s.split('/').pop().split('\\').pop();
  
          // Remove extension if included
          s = s.replace(/\.(png|jpg|jpeg|webp|gif)$/i, '');
  
          // Normalize like IDs: lowercase, underscores, safe chars
          s = s.toLowerCase()
            .replace(/['"]/g,'')
            .replace(/[^a-z0-9\s_-]/g,'')
            .replace(/[\s-]+/g,'_')
        ...
        function computeImagePath(){
          const base = normalizeImageBase(readValue(inImageName));
          return base ? `assets/art/monsters/${base}.png` : '';
        }
  
        function updateImagePreview(){
          if (!imgPreviewEl) return;
          imgPreviewEl.textContent = computeImagePath() || '—';
        }
  ...
  // ---------- output ----------
      ...
      btnClearCamps.addEventListener('click', ()=>{
        ...
        if (inImageName){
          inImageName.addEventListener('input', updateImagePreview);
        }
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', bootMonsterMaker);
  } else {
    bootMonsterMaker();
  }
